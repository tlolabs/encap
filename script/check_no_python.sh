...
# Python files, and deleting a tracked file locally must not bypass the gate.
tracked="$(git ls-files -- '*.py' '*.pyi' '*.pyw' '*.pyx' '*.pyc' '*.pyo' \
  '*.spec' '*.egg-info/*' 'pyproject.toml' 'Pipfile' 'Pipfile.lock' \
  'poetry.lock' 'uv.lock' 'requirements*.txt' 'setup.cfg' 'pyvenv.cfg')"
...
local_files="$(find . \
  -type d \( -name .git -o -name .build -o -path ./build -o -path ./dist \
    ...
    -o -path ./.sparkle -o -path ./.whisper-cpp \) -prune -o \
