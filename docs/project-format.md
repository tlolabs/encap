...
`audio/`, `artwork/`, and `chapters/`. The current manifest schema is version 1.
...
| `transcript_segments` | array | Stable ID, start/end times, speaker, and text. |
...
- A schema version other than 1 is rejected with an explicit compatibility
  error before media is extracted for use or the source document is changed.
- Unknown fields within schema 1 survive a native open/edit/save round trip.
...
Additive schema-1 fields must have safe defaults and must be preserved by older
schema-1 clients. A change that cannot be represented safely under those rules
